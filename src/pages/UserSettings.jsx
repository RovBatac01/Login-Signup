...
const fetchSessionHistory = async () => {
  ...
  // Replace the logSessionEventToServer function around line 82:
  ...
  const logSessionEventToServer = async (type, username) => {
    try {
      ...
      // FIXED: Call the correct session history endpoint
      const response = await axios.post(
        `${API_BASE_URL}/api/session-history/log`, // Updated endpoint
        {
          sessionType: type, // Changed from type to sessionType
  ...
  // Add this useEffect after your existing imports and before the logSessionEventToServer function:
  ...
  useEffect(() => {
    ...
    loadUserData();
    fetchSessionHistory(); // Fetch session history when component loads
  }, []);
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/super-admin/profile`,
      ...
      if (error.response) {
        if (error.response.status === 401) {
          ...
          setMessage(
            error.response.data.message || "Invalid input for profile update."
          );
        } else if (error.response.status === 409) {
          // Added for conflict (username/email in use)
          setMessage(
            error.response.data.message || "Username or email already in use."
          );
        } else {
          setMessage(
  ...
  const handleSavePassword = async (e) => {
    ...
    try {
      // Send data to the backend API with Authorization header
      const response = await axios.post(
        `${API_BASE_URL}/super-admin/change-password`,
      ...
      if (error.response) {
        if (error.response.status === 401) {
          setMessage(
            error.response.data.message ||
              "Unauthorized. Incorrect current password or session expired. Please log in again."
          );
        } else if (error.response.status === 400) {
          setMessage(
            error.response.data.message || "Invalid input for password change."
          );
  ...
  const handleToggleNotifications = async () => {
    ...
    try {
      // Send data to the backend API with Authorization header
      const response = await axios.put(
        `${API_BASE_URL}/super-admin/notifications`,
      ...
      if (error.response) {
        if (error.response.status === 401) {
          ...
          setMessage(
            error.response.data.message ||
              "Invalid input for notification preference."
          );
        } else {
          setMessage(
            error.response.data.message ||
              "Failed to update notification preference due to a server error."
          );
        }
      } else if (error.request) {
        setMessage("No response from server. Check your internet connection.");
        ...
        setMessage(
          "Error setting up request to update notification preference."
        );
      }
      setMessageType("error");
    ...
    <div className={`settings-page-container ${theme}-theme`}>
      ...
      <div className="settings-page-content-wrapper">
        ...
        <div className="settings-page-dashboard">
          {message && (
            <div
              className={`settings-page-message-box ${
                messageType === "success"
            ...
            >
              {messageType === "success" ? (
                ...
                <div className="settings-page-message-content">
                  <AlertCircle
                    size={20}
                    className="settings-page-message-icon"
                  />
                  <span>{message}</span>
                </div>
              )}
          ...
          <section className="settings-page-section-card">
            <h2
              className="settings-page-category-header"
            ...
            >
              ...
              {openSection === "profile" ? (
                <ChevronDown size={20} className="settings-page-toggle-icon" />
              ) : (
                <ChevronRight size={20} className="settings-page-toggle-icon" />
              )}
            </h2>
            <p className="settings-page-category-description">
            ...
            {openSection === "profile" && (
              <div className="settings-page-form-wrapper">
                {/* Edit Profile Item */}
                <div
                  className="settings-page-list-item"
                  onClick={() => {
                  ...
                  <div className="settings-page-item-inner">
                    ...
                    <div className="settings-page-item-action">
                      {showEditProfileForm ? (
                        <ChevronDown
                          size={20}
                          className="settings-page-item-arrow"
                        />
                      ) : (
                        <ChevronRight
                          size={20}
                    ...
                    <div className="settings-page-form-grid">
                      <div className="settings-page-form-group">
                        ...
                        <div className="settings-page-input-container">
                          <User
                            size={18}
                            className="settings-page-input-icon"
                          />
                          <input
                            type="text"
                            id="username"
                  ...
                  >
                    ...
                    <div className="settings-page-password-requirements">
                      <p>
                        <AlertCircle
                          size={16}
                          className="settings-page-info-icon"
                        />{" "}
          ...
          <section className="settings-page-section-card">
            ...
            >
              <div className="settings-page-header-content">
                {theme === "dark" ? (
                  <Moon size={24} className="settings-page-category-icon" />
                ) : (
                  <Sun size={24} className="settings-page-category-icon" />
                )}
              ...
              <div className="settings-page-form-wrapper">
                ...
                <div className="settings-page-theme-toggle">
                  ...
                  >
                    <Sun
                      size={28}
                      className={`settings-page-theme-icon ${
                        theme === "light" ? "active" : ""
                      }`}
                    />
                    <span
                      className={`settings-page-theme-label ${
                        theme === "light" ? "active" : ""
                      }`}
                    >
                      Light Mode
                  ...
                  <label
                    htmlFor="themeToggle"
                    className="settings-page-switch-container"
                  >
                    <input
                      type="checkbox"
                      id="themeToggle"
          ...
          <section className="settings-page-section-card">
            ...
            <p className="settings-page-category-description">
              View your login and logout sessions.
            </p>
            {openSection === "session" && (
              <div className="settings-page-form-wrapper">
                  ...
                  {sessionHistory.length > 0 ? (
                    <div className="settings-page-session-list">
                      {sessionHistory.map((session) => (
                        <div
                          key={session.id}
                          className="settings-page-session-item"
                        >
                          <div
                            className={`settings-page-session-icon ${
                              session.type === "login"
                                ? "login-icon"
                                : "logout-icon"
                            }`}
                          >
                            {session.type === "login" ? (
                              <LogIn
                                size={20}
                                className="settings-page-login-icon"
                              />
                            ) : (
                              <LogOut
                                size={20}
                                className="settings-page-logout-icon"
                              />
                            )}
                          </div>
                          <div className="settings-page-session-details">
                            <div className="settings-page-session-main-info">
                              <span className="settings-page-session-username">
                                {session.username}
                              </span>
                            ...
                            <div className="settings-page-session-secondary-info">
                              <span className="settings-page-session-time">
                                <Clock
                                  size={14}
                                  className="settings-page-session-info-icon"
                                />{" "}
                                {session.timestamp}
                              </span>
                    ...
                    <div className="settings-page-no-sessions">
                      <History
                        size={48}
                        className="settings-page-no-data-icon"
                      />
                      <p>No session history available.</p>

                      <button
                        className="settings-page-button settings-page-button-secondary"
                        onClick={() =>
                          logSessionEventToServer(
                            "login",
                            username || "Test User"
                          )
                        }
