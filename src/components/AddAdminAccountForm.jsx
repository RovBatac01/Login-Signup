...
const AdminCreationForm = ({ onClose, onAddAdmin }) => {
  ...
  useEffect(() => {
    const fetchEstablishments = async () => {
      ...
      try {
        ...
        const response = await axios.get("https://login-signup-production-9bdf.up.railway.app/api/admin/establishments-for-creation");
  ...
  const verifyOtp = async () => {
    ...
    try {
      const response = await axios.post("https://login-signup-production-9bdf.up.railway.app/admin/verify-otp", {
        email: emailForOtp,
        code: otp,
      });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isOtpSent) {
      try {
        const response = await axios.post("https://login-signup-production-9bdf.up.railway.app/admin", {
          username: adminData.username,
          email: adminData.email,
