...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchTotalUsers = async () => {
      ...
      try {
        const response = await fetch('https://login-signup-production-e1ef.up.railway.app/api/total-users');
        if (!response.ok) {
    ...
    const fetchOtherData = async () => {
      ...
      // Fetch total sensors
      try {
        const sensorResponse = await fetch('https://login-signup-production-e1ef.up.railway.app/api/total-sensors');
