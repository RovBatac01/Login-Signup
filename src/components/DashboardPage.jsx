...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchTotalUsers = async () => {
      ...
      try {
        const response = await fetch('https://login-signup-production-9bdf.up.railway.app/api/total-users');
        if (!response.ok) {
    ...
    const fetchOtherData = async () => {
      ...
      // Fetch total sensors
      try {
        const sensorResponse = await fetch('https://login-signup-production-9bdf.up.railway.app/api/total-sensors');
