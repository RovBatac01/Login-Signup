...
const PortableTurbidity = () => {
  ...
  useEffect(() => {
    ...
    const handleTurbidityData = (newData) => {
      console.log("🔄 New Turbidity2 Data:", newData);
      setIsAnimating(true);
    ...
    const fetchLatestTurbidity = async () => {
      try {
        ...
        // prefer turbidity2 if available
        const value = latestData.turbidity2_value ?? latestData.turbidity_value;
        setTurbidityValue(value);
        console.log("📂 Fetched latest turbidity2 value:", value);
  ...
  socket.on("updateTurbidity2Data", handleTurbidityData);

    return () => {
      socket.off("connect", handleConnect);
      socket.off("disconnect", handleDisconnect);
      socket.off("updateTurbidity2Data", handleTurbidityData);
    };
  }, []);
